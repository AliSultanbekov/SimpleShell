...
static int handle_aliased_cmd(char *args[BUF_SIZE], uint32_t argc);

static int handle_cmd(char *args[BUF_SIZE], uint32_t argc)
{
    BUG_ON(argc == 0);
    ...
    if (alias_getindex(args[0]) != -1) {
        return handle_aliased_cmd(args, argc);
    }

    if (cmd[0] == '!')
    {
        return handle_history_request(args, argc, cmd);
    }
...
static int handle_aliased_cmd(char *args[BUF_SIZE], uint32_t argc) {
    ...
    int ret = handle_cmd(d.args, d.argc);
    free(e);
    return ret;
}
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        /* take input buffer, and pass it to the processor */
